...
`restaurant_name` VARCHAR( 128 ),
`restaurant_postal_code` INT( 16 ),
`review_user_email` VARCHAR( 32 ) ,
`updated_on` TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NOT
NULL DEFAULT CURRENT_TIMESTAMP,
`review_updated_on` TIMESTAMP,
FOREIGN KEY (`restaurant_name`, `restaurant_postal_code`, `review_updated_on`, `review_user_email`)
REFERENCES reviews(`restaurant_name`, `restaurant_postal_code`, `updated_on`,`user_email`)
